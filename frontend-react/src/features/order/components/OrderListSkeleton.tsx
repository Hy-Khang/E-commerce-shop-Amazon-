  ...
  return (
    <div className="space-y-4">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="animate-pulse rounded-xl border border-border-default bg-white p-4">
            ...
            {Array.from({ length: 3 }).map((_, j) => (
              <div key={j} className="flex items-center gap-3">
                ...
                <div className="space-y-1.5 text-right">
                  <div className="ml-auto h-3 w-8 rounded bg-neutral-200" />
                  <div className="ml-auto h-3.5 w-16 rounded bg-neutral-200" />
                </div>
              </div>
            ))}
          </div>
