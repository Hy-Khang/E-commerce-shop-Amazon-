...
export function PaymentTransactionList({ orderId, variant = 'customer' }: Props) {
  ...
  const titleClass = isAdmin ? 'text-slate-900' : 'text-text-primary';
  const labelClass = isAdmin ? 'text-slate-500' : 'text-text-secondary';
  const valueClass = isAdmin ? 'text-slate-900' : 'text-text-primary';
