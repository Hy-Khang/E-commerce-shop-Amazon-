...
- **CartModule / CartService** — reads the caller's cart for `GET /coupons/available` (voucher picker). Acyclic: CartModule imports only ProductModule, never CouponModule.
...
- **Admin lock (`admin_disabled`, Phase 2):** admin `DELETE` on a shop coupon sets the sticky `admin_disabled` flag. A locked coupon validates as inactive (`COUPON_006`) and the seller's `updateSellerCoupon` rejects it (`COUPON_013`). Platform coupons never set the flag.
- **Unlock ≠ activate (Phase 3):** `unlockCoupon` clears **only** `admin_disabled` and leaves `is_active` untouched — unlocking is "stop moderating", not "activate on the seller's behalf". After unlock the coupon stays inactive until the owning seller re-enables it.
- **Update-path scope guard (Phase 3):** the create/update DTOs' `@ArrayMinSize(1)` only fires when `scope` is present in the body, so `{ product_ids: [] }` (or `{ category_ids: [] }`) alone would slip through and strip a scoped coupon to zero targets → a permanently dead coupon. `updateSellerCoupon` and `updateCoupon` recompute the *effective* scope+targets after the patch and reject an empty products/categories set (`VALIDATION_001`). Create is already covered by the DTO.
- **Shop-slug prefix is stable:** `shops.slug` is immutable after creation (seller shop `PATCH` cannot change it), so the `<slug>-<CODE>` prefix on a shop coupon never drifts — no re-sync needed.
- **Voucher availability catalog (Phase 4):** `GET /coupons/available` → `getAvailableCouponsForCart(userId)` builds the selectable-voucher list for the current cart. `CouponRepository.findAvailableForCart(shopIds, now)` applies the **invisible** hard gates (active, not `admin_disabled`, inside inclusive window, not globally exhausted, shop active for shop coupons) so rejected-for-invisible-reason coupons never appear; empty `shopIds` → platform only (no `IN ()`). The service then layers the **cart-dependent** gates per coupon — reusing `getApplicableTotalsByShop` + `calculateDiscount` — and tags each with `eligible` + `reason ∈ { below_min, no_applicable_items, user_limit }`. This mirrors `validateCoupon` + `validateAndCalculateDiscounts` exactly, so an `eligible` coupon is guaranteed to pass checkout re-validation at that instant. **Advisory only** (like preview): no reservation, `POST /orders/preview` / checkout decide the real allocation and remain the source of truth. Empty cart → `{ platform: [], shops: [] }` (never 400 — `CartEmptyException` is caught). A coupon the user selected then lost (expired/exhausted) is caught by preview's existing `COUPON_0xx` gating on the checkout page; no extra handling needed.
