...
## Purpose
Provides aggregated analytics for the admin dashboard — summary stats, revenue trends, order distribution, top products, and low stock alerts.

## Entities
None owned. Queries across: `orders`, `order_items`, `users`, `roles`, `products`, `product_variants`.

## Endpoint
- `GET /admin/dashboard` — returns all dashboard sections via `Promise.allSettled()` (partial failure safe)
