...
- **Validation:** `dto/decoration-config.dto.ts` — nested class-validator DTOs + a custom `@ValidatorConstraint` (`IsBlockData`) that validates `block.data` against the DTO matching the sibling `block.type` (discriminator can't reach a sibling field; repo precedent is `common/validators/is-image-path`). Limits: ≤20 blocks, unique ids, hero 1–5 images, grid 1–12 unique ids, `best_sellers` limit ∈ `[4,8,12]`; serialized ≤16 KB enforced in `updateMyShop` → `SHOP_006`.
- **Auto best-sellers block (`best_sellers`):** stores no product ids (`{ title?, limit?, columns? }`) — the FE hydrates it via the existing `GET /products?shop_id=&sort=best_selling`. No BE change beyond accepting the new block type on save (`BLOCK_TYPES` + `BLOCK_DATA_DTOS` + `DECORATION_LIMITS.BEST_SELLERS_LIMITS`). Quick-start templates + rich preview are FE-only (no BE surface).
