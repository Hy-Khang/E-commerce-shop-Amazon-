...
export class ShipperOrderController {
  ...
  @ApiOperation({
    summary: 'List orders for shipper (available or my deliveries)',
  })
  @ApiResponse({
    status: 200,
  ...
  @Permissions(PERMISSIONS.ORDERS_READ)
  @ApiOperation({
    summary: 'Get order detail (assigned to shipper or available)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description: 'ORDER_003: Already assigned or not confirmed',
  })
  async acceptOrder(
    @CurrentUser() user: ICurrentUser,
    @Param('id', ParseIntPipe) id: number,
  ...
  @ApiResponse({
    status: 400,
    description: 'ORDER_003: Invalid transition or unpaid',
  })
  @ApiResponse({
    status: 403,
    description: 'ORDER_004: Not assigned to this shipper',
  })
  async markDelivered(
    @CurrentUser() user: ICurrentUser,
    @Param('id', ParseIntPipe) id: number,
  ...
  @ApiOperation({
    summary: 'Update shipper location for order (append-only, 30s rate limit)',
  })
  @ApiResponse({ status: 204, description: 'Location updated' })
  @ApiResponse({
    status: 400,
    description: 'ORDER_003: Not shipping / rate limited',
  })
  @ApiResponse({
    status: 403,
    description: 'ORDER_004: Not assigned to this shipper',
  ...
  @Permissions(PERMISSIONS.ORDERS_READ)
  @ApiOperation({
    summary: 'Get order tracking for shipper (timeline + locations)',
  })
  @ApiResponse({
    status: 200,
