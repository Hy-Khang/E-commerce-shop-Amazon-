...
## Event Dependencies
- Listens to: `order.status_updated` (emitted by order.service.ts from admin/seller status changes)
...
- **No self-notification** — customer-initiated actions (placing order, cancelling own order) do not create notifications.
