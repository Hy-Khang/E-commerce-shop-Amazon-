...
describe('ToolDispatcher', () => {
  ...
  beforeEach(() => {
    productService = {
      findActiveProducts: jest.fn().mockResolvedValue({
        data: [{ id: 1, name: 'Áo', slug: 'ao', variants: [{ id: 11, price: 100, sale_price: null, stock_quantity: 3 }] }],
        meta: {},
      }),
    ...
    orderService = {
      previewCheckout: jest.fn().mockResolvedValue({
        subtotal: 100, discount_total: 0, coin_discount: 0, coins_applied: 0,
        shipping_total: 0, grand_total: 100, shops: [], applied_coupons: [],
      }),
      checkout: jest.fn(),
    ...
    dispatcher = new ToolDispatcher(
      productService, cartService, orderService, userProfileService,
    );
  });
  ...
  it('gates checkout/orders behind login for guests (needs_login, no service call)', async () => {
    const checkout = await dispatcher.run(AGENT_TOOL_NAMES.PROPOSE_CHECKOUT, {}, guest);
    const orders = await dispatcher.run(AGENT_TOOL_NAMES.LIST_ORDERS, {}, guest);
  ...
  it('answers policy FAQ from static content (no service)', async () => {
    const res = await dispatcher.run(AGENT_TOOL_NAMES.GET_POLICIES, { topic: 'returns' }, guest);
