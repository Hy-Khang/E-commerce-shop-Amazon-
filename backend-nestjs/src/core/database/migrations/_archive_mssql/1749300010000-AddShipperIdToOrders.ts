...
export class AddShipperIdToOrders1749300010000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE orders ADD shipper_id INT NULL`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP INDEX idx_orders_shipper_id ON orders`);
    await queryRunner.query(
      `ALTER TABLE orders DROP CONSTRAINT fk_orders_shipper_id`,
    );
    await queryRunner.query(`ALTER TABLE orders DROP COLUMN shipper_id`);
  }
}
