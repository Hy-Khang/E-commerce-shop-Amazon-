...
export class AddAuthSecurityColumns1749300008000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE users ADD email_verify_count INT NOT NULL CONSTRAINT df_users_email_verify_count DEFAULT 0`,
    );
    await queryRunner.query(
      `ALTER TABLE users ADD email_verify_count_reset DATETIME2 NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE users ADD email_verify_attempts INT NOT NULL CONSTRAINT df_users_email_verify_attempts DEFAULT 0`,
    );

    // 3. Add password reset columns
    await queryRunner.query(
      `ALTER TABLE users ADD password_reset_token_hash NVARCHAR(255) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE users ADD password_reset_expires_at DATETIME2 NULL`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX idx_oauth_codes_code_hash ON oauth_codes`,
    );
    ...
    await queryRunner.query(
      `DROP INDEX idx_users_password_reset_token_hash ON users`,
    );
    await queryRunner.query(`DROP INDEX idx_users_email_verify_token ON users`);

    // Drop password reset columns
    await queryRunner.query(
      `ALTER TABLE users DROP CONSTRAINT df_users_email_verify_attempts`,
    );
    await queryRunner.query(
      `ALTER TABLE users DROP CONSTRAINT df_users_email_verify_count`,
    );
    await queryRunner.query(
      `ALTER TABLE users DROP CONSTRAINT df_users_email_verified`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE users DROP COLUMN email_verify_attempts`,
    );
    await queryRunner.query(
      `ALTER TABLE users DROP COLUMN email_verify_count_reset`,
    );
