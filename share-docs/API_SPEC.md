...
|--------|------|-------------|------------|
| GET | `/seller/shop` | Get current seller's shop (incl. parsed `decoration_config` + `pickup_address`/`latitude`/`longitude`) | `shops:read` |
| POST | `/seller/shop` | Create shop (one per seller; slug auto-generated, immutable) | `shops:create` |
| PATCH | `/seller/shop` | Update shop (name, description, logo_url, banner_url, `pickup_address`/`latitude`/`longitude`, `decoration_config`; slug immutable) | `shops:update` |
...
>
> **`POST /orders/preview`** returns `CheckoutPreviewResponseDto { subtotal, discount_total, coin_discount, coins_applied, shipping_total, grand_total, shops[], applied_coupons[] }`. Body is `{ coupon_code?, coupon_codes?, coins_to_redeem?, address_id? }` (same coupon fields as checkout, plus Xu, plus the selected address). It is **advisory, exact-at-the-time — NOT a reservation**: it writes nothing (no `coupon_usages`, no coin consumption, no stock/usage hold), and `POST /orders` re-validates and remains the sole source of truth (a coupon may run out, or the balance change, in between). Invalid coupons return the same `COUPON_0xx` errors as checkout.
>
> **Shipping fee — distance-based (H1):** Each sub-order's `shipping_fee` is computed per shop from the great-circle (Haversine) distance between the shop's pickup point (`shops.latitude/longitude`) and the delivery address (`addresses.latitude/longitude`): a flat base covers the first 5 km, then a per-km fee, rounded to the nearest 1.000 ₫ and clamped to `[15.000, 60.000]`. Coordinate-only → immune to the 2025 province merger (no carrier API / administrative codes). When either side has **no coordinates** (legacy/manual address, un-located shop) it falls back to the flat `30.000 ₫`. `checkout` and `preview` call the **same `ShippingService.computeShippingByShop`**, so the estimate matches what is charged — but `preview` only has the address when `address_id` is sent (omit → flat fallback in the estimate). A carrier-API strategy (GHN/GHTK) can later wrap this resolver (try provider → distance → flat) without touching checkout/preview.
>
...
> **Order assignment model:** First-come-first-served. Shipper calls `/accept` on a `confirmed` order with no shipper assigned. Atomic conditional UPDATE prevents race conditions — if two shippers accept simultaneously, only one succeeds; the other receives `ORDER_003 (400)`. On a successful accept, if the order's shop has a pickup `latitude`/`longitude` set, the first tracking point is seeded there (best-effort) so the package starts on the map at the shop before the first manual `/location` update.
>
